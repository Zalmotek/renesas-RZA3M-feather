...
void init_sensor(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        Serial.print((uint8_t *)"** R_IIC_MASTER_Open API failed ** \r\n");
        i2cok = false;
        //APP_ERR_TRAP(err);
    }
    ...
    if ( (FSP_SUCCESS == err) && (DEVICE_SIGNATURE == device_id) )
    {
        ...
        err = R_RIIC_MASTER_Write(&g_i2c_master0_ctrl, measure_enable_payload, TWO_BYTE, false);
        if (FSP_SUCCESS == err)
        {
            validate_i2c_event();
            /* handle error */
            ...
            {
                Serial.print((uint8_t *)"** POWER_CTL reg I2C write failed!!! ** \r\n");
                i2cok = false;
                //APP_ERR_TRAP(err);
            }
        }
        ...
        else
        {
            ...
            Serial.print((uint8_t *)"** R_IIC_MASTER_Write API failed ** \r\n");
            i2cok = false;
            //APP_ERR_TRAP(err);
        }
    }
    ...
    else
    {
        ...
        Serial.print((uint8_t *)"Error in reading Device ID ** \r\n");
        i2cok = false;
        //APP_ERR_TRAP(err);
    }
}
...
void read_sensor_data(uint8_t *xyz_data)
{
    ...
    if(NULL == xyz_data)
    {
        ...
        Serial.print((uint8_t *)"** NULL Pointer check fail ** \r\n");
        i2cok = false;
        //APP_ERR_TRAP(err);
        return;
    }
    /* Write Accelerometer Data register   */
    err = R_RIIC_MASTER_Write(&g_i2c_master0_ctrl, &internal_reg, ONE_BYTE, true);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        Serial.print((uint8_t *)"** R_IIC_MASTER_Write API failed ** \r\n");
    ...
    else
    {
        ...
        if (FSP_SUCCESS == err)
        {
            err = R_RIIC_MASTER_Read(&g_i2c_master0_ctrl, &adxl_data[0], DATA_REGISTERS, false);
            /* handle error */
            if (FSP_SUCCESS != err)
            {
                Serial.print((uint8_t*)"** R_IIC_MASTER_Read API failed ** \r\n");
                i2cok = false;
                //APP_ERR_TRAP(err);
                return;
            }
            else
            {
                ...
                if (FSP_SUCCESS != err)
                {
                    Serial.print((uint8_t*)"** Data register, I2C read failed ** \r\n");
                    i2cok = false;
                    //APP_ERR_TRAP(err);
                    return;
                }
        ...
        {
            Serial.print((uint8_t*)"** Accel Data register, I2C write failed ** \r\n");
            i2cok = false;
            //APP_ERR_TRAP(err);
            return;
        }
...
static fsp_err_t get_device_id(uint8_t *dev_id)
{
    ...
    err = R_RIIC_MASTER_Write(&g_i2c_master0_ctrl, &reg_address, ONE_BYTE, true);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        Serial.print((uint8_t*)"** R_IIC_MASTER_Write API failed ** \r\n");
    ...
    else
    {
        ...
        if(FSP_ERR_TRANSFER_ABORTED == err)
        {
            Serial.print((uint8_t*)"** Device ID reg, I2C write failed ** \r\n");
            //APP_ERR_TRAP(err);
            i2cok = false;
            return err;
        }
        else
        {
            ...
            err  = R_RIIC_MASTER_Read(&g_i2c_master0_ctrl, dev_id, ONE_BYTE, false);
            /* handle error */
            if (FSP_SUCCESS != err)
            {
                Serial.print((uint8_t*)"** R_IIC_MASTER_Read API failed ** \r\n");
            ...
            else
            {
                ...
                if(FSP_ERR_TRANSFER_ABORTED == err)
                {
                    Serial.print((uint8_t*)"** Device ID read,  I2C read failed ** \r\n");
                    //APP_ERR_TRAP(err);
                    i2cok = false;
                    return err;
                }
